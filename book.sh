...
then
  tmux new-session -d -s book   -n shell   -c ~/Development/code/handybook

  tmux new-window     -t book:5 -n server  -c ~/Development/code/handybook
  tmux new-window     -t book:6 -n git     -c ~/Development/code/handybook
